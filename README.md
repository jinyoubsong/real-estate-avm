...
브라우저에서 `http://localhost:8000` 접속 → 시도/법정동/지번을 입력하고 **"건물동/호수 조회"** 버튼을 누르면:

- 건축물대장 표제부에서 그 지번에 있는 건물동 목록을 가져와 드롭다운으로 보여줌 (상가/경비실 등 부속동 포함)
- 건물 용도(예: "아파트", "오피스텔")를 보고 **부동산 유형을 자동으로 추천**해 선택해 둠
- 건물동을 고르면 전유부에서 해당 동의 호실 목록(호수·전용면적·층)을 드롭다운으로 보여줌
- 호수까지 고르면 그 호실의 전용면적/층이 확정됨

이후 **면적/건축년도를 입력하면**: 그 값 그대로 사용하고, **비워두면**: 제출 시 선택한 위치(+건물동/호수)로 건축물대장을 다시 조회해 채웁니다.
- 건물동/호수가 없으면(드롭다운으로 안 골랐거나 단독/다가구처럼 호실 구분이 없으면) 표제부의 건물 전체 연면적/지상층수를 참고치로 사용
- 자동조회에 실패하면(API 미승인, 주소 못 찾음 등) 오류 안내와 함께 직접 입력을 요청함
- 드롭다운 없이 직접 타이핑할 때도 "104동"/"1301호"처럼 접미사를 붙이거나 안 붙여도("104"/"1301") 매칭됨
